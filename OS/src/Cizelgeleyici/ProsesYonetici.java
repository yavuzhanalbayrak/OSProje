...
public class ProsesYonetici {
	...
	public void ProsesAskida(int saniye , ProsesYonetici p, int kontrolId,int kontrolOncelik,int kontrolPatlama,int kontrolVaris) throws IOException {
		List<String> params = java.util.Arrays.asList( "java", "-jar","ProManag.jar",Integer.toString(saniye),Integer.toString(kontrolId),
				Integer.toString(kontrolOncelik),Integer.toString(kontrolPatlama),"askida");
				ProcessBuilder builder = new ProcessBuilder(params);
				process=builder.start();
				Scanner scanner = new Scanner(process.getInputStream());
				while (scanner.hasNextLine()) {
